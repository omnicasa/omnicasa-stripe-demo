...
const SetupForm = ({ customerId }: { customerId: string }) => {
    ...
    return (
        <form onSubmit={handleSubmit}>
            <PaymentElement />
            <Button className="mt-4" disabled={!stripe}>Submit</Button>
...
const Payment = () => {
    ...
    useEffect(() => {
        (async () => {
            const response = await fetch(`/api/future-payment?customer_id=${customerId}`, { method: 'POST' });
    ...
    return (
        <StripeWrapper clientSecret={clientSecret}>
            <UiWrapper containerClassName="space-y-6">
                <h1 className="text-4xl font-bold text-gray-800">Omnicasa Recurring payments</h1>
                {hasError && (
                    <div className="p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg dark:bg-red-200 dark:text-red-800" role="alert">
                        <span className="font-medium">Error</span> Something went wrong while submitting your details. Please try again later.
                    </div>
                )}
                <h3 className="text-md text-gray-600">For customer ID: {clientSecret}</h3>
